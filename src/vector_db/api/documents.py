...
async def get_all_documents(
    ...
    return await document_service.get_documents_paginated(
        library_id=library_id, skip=skip, limit=limit
    )


@router.put("/{document_id}", response_model=Document, status_code=status.HTTP_200_OK)
async def update_document(document_id: UUID, document_update: DocumentUpdate) -> Document:
    """Update a document."""
...
async def get_document_chunks(document_id: UUID) -> list[Chunk]:
    """Get all chunks in a document."""
    return await document_service.get_chunks(document_id)
